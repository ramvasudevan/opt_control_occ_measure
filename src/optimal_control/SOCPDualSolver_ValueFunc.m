function [out] = SOCPDualSolver_ValueFunc( t, x, u, f, g, sigma, dl0, hX, hXT, hU, h, H, degree, opts )
% 
% dx = [ f(t,x) + g(t,x) u ] * dt + sigma * dw
% 
% Dual problem:
% 
% sup  v( 0, x0 )
% s.t. Av + h >= 0,      on [0,T] x X x U,     dual: mu
...
Hess = diff( diff(v,x)', x );
Av = diff(v,t) + diff(v,x) * ( f + g * u ) + 0.5 * sigma' * Hess * sigma;
...
% LFv + h >= 0 : mu
prog = sosOnK( prog, Av + h, [ t; x; u ], [ hT; hX; hU ], degree );
