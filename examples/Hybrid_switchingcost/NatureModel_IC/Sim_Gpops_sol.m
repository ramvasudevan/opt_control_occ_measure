...
% xs = [0.35; 0; 0; 0.8376];
% load('Result_Gpops_T3_noIC.mat');

xs = [0.35; 0; 0; 0.7];
load('Result_Gpops_T7_noIC.mat');

figure(8);
hold on;
plot(x_hist, y_hist, 'LineWidth',4);
...
controller{1} = @(tt,xx) gpops_controller( tt, xx, T, t_hist_gpops, control_hist_gpops );
controller{2} = @(tt,xx) gpops_controller( tt, xx, T, t_hist_gpops, control_hist_gpops );
...
function out = gpops_controller( tt, xx, T, t_hist_gpops, control_hist_gpops )
    [~, idx] = min( abs( tt * T - t_hist_gpops ) );
