...
c{2,1} = (xvar(1) * polysin(xvar(3)) + l0 * polysin(-al) - d_des) ^ 2;        % step length = l * sin(theta) + l0 * sin(-alpha)
h{2} = uvar^2 * T;
H{2} = msspoly(0);
...
fprintf('Finished solving fixedIC, T%d, deg %d, F approx 3.\n', T, d);
save(['Ram_Result_Fpoly3_T',num2str(T),'_fixedIC_deg',num2str(d)]);