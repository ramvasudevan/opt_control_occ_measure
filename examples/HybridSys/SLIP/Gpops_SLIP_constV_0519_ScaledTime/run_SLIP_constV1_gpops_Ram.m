...
params.domain{1} =...
        [ 0.1, 0.2;         % l         - leg length
         -0.3, 0.5;         % l_dot     - time derivative of l
           -1, 1;           % theta     - leg angle
...
for iphase = 1 : nphases
    ...
    if iphase == 1
        ...
        bounds.phase(iphase).initialtime.upper = 1; 
    end
    if iphase == nphases
...
mesh.tolerance       = 1e-5;
for i = 1 : nphases
    mesh.phase(i).colpoints = 10 * ones(1,20);
    mesh.phase(i).fraction = 0.05 * ones(1,20);
end
...
setup.functions.continuous           = @SLIPContinuous_constV1_fixedT_ScaledTime;
setup.functions.endpoint             = @SLIPEndpoint_constV1_fixedT_ScaledTime;
