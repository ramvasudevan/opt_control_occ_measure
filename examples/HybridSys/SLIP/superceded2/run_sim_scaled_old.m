...
if opt(3)
    ...
    R_13 = @(xx) rescale_reset(@Reset_S2F, xx, scale_x{1}, trans_x{1}, scale_x{3}, trans_x{3});
    ...
    R_13 = @(xx) rescale_reset(@Reset_S2F_Approx, xx, scale_x{1}, trans_x{1}, scale_x{3}, trans_x{3});
    R_23 = @(xx) rescale_reset(@(var) var, xx, scale_x{2}, trans_x{2}, scale_x{3}, trans_x{3});
    ...
    switch current_mode
        case 1      % Stance
            ...
            options = odeset(options,'AbsTol',1e-9,'RelTol',1e-8);
                ...
                ode45(@(t,x) ( f{1}(x) + g{1}(x) * controller(t,x) ), ...
        ...
        case 2      % Flight 1 (under ground)
            options = odeset('Events',@EvtFunc_F1_scaled_old);
            options = odeset(options,'AbsTol',1e-9,'RelTol',1e-8);
            [ tout, xout, event_time, event_state, event_id ] = ...
                ode45(@(t,x) ( f{2}(x) ), ...
        ...
        case 3      % Flight 2 (above ground)
            options = odeset('Events',@EvtFunc_F2S_scaled_old);
            options = odeset(options,'AbsTol',1e-9,'RelTol',1e-8);
            [ tout, xout, event_time, event_state, event_id ] = ...
                ode45(@(t,x) ( f{3}(x) ), ...
                             (current_time : 0.01 : MaxTime), x0, options);
