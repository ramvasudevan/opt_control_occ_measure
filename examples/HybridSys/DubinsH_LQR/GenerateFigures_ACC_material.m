...
ylabel('$u_1(t)$','Interpreter','LaTex','FontSize',30);
xlim([0,3]);
ylim([0,1.1]);
...
xlabel('$t$','Interpreter','LaTex','FontSize',30);
ylabel('$u_2(t)$','Interpreter','LaTex','FontSize',30);
xlim([0,3]);
ylim([-3.2,3.2]);
set(gca,'XTick',[0,1,2,3]);
set(gca,'YTick',[-3,0,3]);
...
%% Plot our result
% [tc1,state] = plotdata('data_old/d6_1e5_v2w2',scaling,  8,  [.988,.553,.349]);
% tc2 = plotdata('data_old/d8_1e5_v2w2',scaling,   6,  [.890,.290,.2]);
% tc3 = plotdata('data_old/d10_1e5_v2w2',scaling,   4, [.702,0,0] );

%% Plot groundtruth result (gpops)
% load('gpops_data');
% figure(1);
% plot(state(:,1), state(:,2),'LineWidth', 2, 'color', 'b');
% 
% figure(2);
% % subplot(1,2,1);
% plot(time, control(:,1), 'LineWidth', 2, 'color', 'b');
% % subplot(1,2,2);
% figure(3);
% plot(time, control(:,2), 'LineWidth', 2, 'color', 'b');
...
% Modes
figure(1);
% legend('Degree 6','Degree 8', 'Degree 10','Optimal solution');
...
figure(2);
% legend('Optimal solution','2k = 10','2k=8','2k=6');
% subplot(1,2,1);
