...
% The reset map is more complicated: R(x,y,theta) = -y^2 + 0.5
% -------
% |  1  |    |
% |-----|    | <- 2
% |  3  |    |
...
% bounds.phase(iphase).duration.upper = 3;
...
bounds.phase(iphase).finaltime.lower = t0;
bounds.phase(iphase).finaltime.upper = T;
bounds.phase(iphase).initialstate.lower = [ -1 ];
bounds.phase(iphase).initialstate.upper = [ 1 ];
bounds.phase(iphase).state.lower = [ -1 ];
bounds.phase(iphase).state.upper = [ 1 ];
bounds.phase(iphase).finalstate.lower = [ -1 ];
bounds.phase(iphase).finalstate.upper = [ 1 ];
bounds.phase(iphase).control.lower = [ -10 ];
bounds.phase(iphase).control.upper = [ 10 ];
bounds.phase(iphase).integral.lower = 0;
bounds.phase(iphase).integral.upper = 100000;
% bounds.phase(iphase).duration.lower = 1e-6;
% bounds.phase(iphase).duration.upper = 3;
...
bounds.eventgroup(2).lower = zeros(1,5);
bounds.eventgroup(2).upper = zeros(1,5);
...
setup.functions.continuous           = @ShortcutContinuous;
setup.functions.endpoint             = @ShortcutEndpoint;
